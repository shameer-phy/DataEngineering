...
DROP SCHEMA IF EXISTS `rent-a-film`;
CREATE SCHEMA `rent-a-film`;
USE `rent-a-film`;
...
GROUP_CONCAT(DISTINCT CONCAT(c.name, ': ',
		(SELECT GROUP_CONCAT(f.title ORDER BY f.title SEPARATOR ', ')
                    FROM`rent-a-film`.film f
                    INNER JOIN`rent-a-film`.film_category fc
                      ON f.film_id = fc.film_id
                    INNER JOIN`rent-a-film`.film_actor fa
...
AS film_info
FROM`rent-a-film`.actor a
LEFT JOIN`rent-a-film`.film_actor fa
  ON a.actor_id = fa.actor_id
LEFT JOIN`rent-a-film`.film_category fc
  ON fa.film_id = fc.film_id
LEFT JOIN`rent-a-film`.category c
